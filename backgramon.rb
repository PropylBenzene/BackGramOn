...
user_name = [ARGV[0]]#[ARGV[0]] # User ID
...
#This pulls the items within the file for the listed IG Handle per handle name.
file = File.open("#{real_name}.txt", "a+")
contents = file.read
file.close
text = contents.split("\n\n\n")
...
temp_post_text.each do |i|
log = File.open("#{real_name}.txt", "a+")
entry = i + "\n\n\n\n"
log.puts entry
log.close
end
